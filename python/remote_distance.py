...
if clientID!=-1:
    ...
    #获得相机句柄
    _, sensorHandle = vrep.simxGetObjectHandle(clientID, 'Vision_sensor#', vrep.simx_opmode_oneshot_wait)
    _, sphereHandle = vrep.simxGetObjectHandle(clientID, 'Plane', vrep.simx_opmode_oneshot_wait)
    #获得深度相机的近景和远景距离
    ...
    while (vrep.simxGetConnectionId(clientID) != -1):
        ...
        if err == vrep.simx_return_ok:
            ...
            img_s=cv2.cvtColor(img,cv2.COLOR_BGR2HSV)[:,:,1]
            region=np.where(img_s>250)
            index=np.array(region[0]).argmax(0)
            
            
            xmin,xmax=min(region[0]),max(region[0])
            ymin,ymax=min(region[1]),max(region[1])
            w,h=xmax-xmin,ymax-ymin
            cv2.rectangle(img, (ymin,xmin), (ymax,xmax), (0,255,0), 2)
            # cv2.line(img,(ymin,xmax),(ymax,xmax),(255,0,0),2)
