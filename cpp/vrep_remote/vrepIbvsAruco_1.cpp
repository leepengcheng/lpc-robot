...
void setRobotjointSpeed(vpColVector &v) {
    ...
    simxCallScriptFunction(clientID, VREP_COMMANDER_SERVER, sim_scripttype_childscript, "setRobotJointPosition", 0,
                           NULL, inFloatCnt, inFloats, 0, NULL, 0, NULL, NULL, NULL,
...
vpMatrix getRobotJacobianMatrix() {
    simxInt retFloatCnt;
    simxFloat *retFloats;
    int result = simxCallScriptFunction(clientID, VREP_COMMANDER_SERVER, sim_scripttype_childscript,
                                        "getJacobianfunction", 0, NULL, 0, NULL, 0, NULL, 0, NULL, NULL, NULL,
...
int main() {
    ...
    simxFinish(-1); //关闭所有连接
    clientID = simxStart((simxChar *) "127.0.0.1", -1, true, true, 2000, 5); //打开连接
    if (clientID == -1) {
    ...
    //获得深度相机的近景和远景距离
    float nearClip = 0;
    simxGetObjectFloatParameter(clientID, camHandle, sim_visionfloatparam_near_clipping, &nearClip,
                                simx_opmode_oneshot_wait);
    float farClip = 0;
    simxGetObjectFloatParameter(clientID, camHandle, sim_visionfloatparam_far_clipping, &farClip,
                                simx_opmode_oneshot_wait);

    int resolution[2];//获取传感器的分辨率
    simxGetObjectIntParameter(clientID, camHandle, sim_visionintparam_resolution_x, &resolution[0],
                              simx_opmode_oneshot_wait);
    simxGetObjectIntParameter(clientID, camHandle, sim_visionintparam_resolution_y, &resolution[1],
                              simx_opmode_oneshot_wait);
    ...
    float angle = 0;
    simxGetObjectFloatParameter(clientID, camHandle, sim_visionfloatparam_perspective_angle, &angle,
                                simx_opmode_oneshot_wait);
