...
function common.createCustomUi(nakedXml,title,dlgPos,closeable,onCloseFunction,modal,resizable,activate,additionalAttributes,dlgSize)
...
function common.getSelectedEditWidget(ui)
    local ret=-1
...
function common.setSelectedEditWidget(ui,id)
    if id>=0 then
        simUI.setCurrentEditWidget(ui,id)
    end
end

function common.getRadiobuttonValFromBool(b)
    if b then
        return 1
    end
    return 0
...
function common.getCheckboxValFromBool(b)
    if b then
        return 2
    end
    return 0
...
function common.writeSessionPersistentObjectData(objectHandle,dataName,...)
    local data={...}
...
function common.readSessionPersistentObjectData(objectHandle,dataName)
    local nm="___"..sim.getScriptHandle()..sim.getObjectName(objectHandle)..sim.getInt32Parameter(sim.intparam_scene_unique_id)..sim.getObjectStringParameter(objectHandle,sim.objstringparam_dna)..dataName
...
function common.fastIdleLoop(enable)
...
function common.isPluginLoaded(pluginName)
    local index=0
    local moduleName=''
    while moduleName do
