...
function getPlaningPath(targetMatrix)
    -- path,lengths=TOOLS:loadPath(rdsHandle,'jacoPath_1')     --加载保存的路径
    --进行路径规划
...
--         if command == TOOLS.REPLAN then
--             sim.rmlRemove(rmlHandle)
--             return 1
--         end
--     until (command == TOOLS.START)
...
function replaningAndExcuteMotion(targetMatrix,maxCount)
    ...
    for i=1,count do
        ...
        if path then
            ...
            TOOLS:savePath(rdsHandle,"jacoPath_1", path, lengths)--保存路径
...
function sysCall_threadmain()
    ...
    -- sim.setIntegerSignal("command", TOOLS.START) --初始化启动信号
    ...
    local mat=sim.getObjectMatrix(pickTargets[1],-1)
    local targetMatrix = TOOLS:getTransMatrix(mat, {0, 0, -0.1})
    ...
    targetMatrix=TOOLS:getTransMatrix(mat,{0,0,-0.02})
    local path,lengths=generateIkPath(targetMatrix,20,true) --不考虑避障
    if path then
        executeMotion(path,lengths,maxVel,maxAcc,maxJerk)
    end
    ...
    targetMatrix=TOOLS:getTransMatrix(mat,{0,0,-0.1})
    ...
    -- local targetMatrix = TOOLS:getTransMatrix(pickTargets[1], {-0.5, -0.1, -0.2})
    -- replaningAndExcuteMotion(targetMatrix)
