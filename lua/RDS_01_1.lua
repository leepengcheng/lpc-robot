...
findCollisionFreeConfig=function(matrix)
    ...
    local c=sim.getConfigForTipPose(ikPINV,jh,0.65,100,nil,collisionPairs,nil,jointLimitsL,jointRanges)
    return c
end
...
generateIkPath=function(startConfig,goalPose,steps,ignoreCollisions)
    ...
    local c=sim.generateIkPath(ikPINV,jh,steps,coll)
            ...
            if (res>=0) then
                ...
                for i=1,#lengths-1,1 do
                    ...
                    if (l>=l1)and(l<=l2) then
                        t=(l-l1)/(l2-l1)
                        for j=1,6,1 do
                            q=path[6*(i-1)+j]+getJointPosDifference(path[6*(i-1)+j],path[6*i+j],jt[j]==sim.joint_revolute_subtype)*t
                            dq=getJointPosDifference(previousQ[j],q,jt[j]==sim.joint_revolute_subtype)
                            previousQ[j]=q
        ...
        if (res>=0) then
            ...
            for i=1,#lengths-1,1 do
                ...
                if (l>=l1)and(l<=l2) then
                    t=(l-l1)/(l2-l1)
                    for j=1,6,1 do
                        jointPos[j]=path[6*(i-1)+j]+getJointPosDifference(path[6*(i-1)+j],path[6*i+j],jt[j]==sim.joint_revolute_subtype)*t
                    end
                    applyJoints(jh,jointPos)
                    break
                end
            end
        end
