...
doCalculation=function()
    local divisions=10 -- the divisions used for each joint. The total nb of pts becomes (divisions+1)^7
    ...
    local randomSteps=100000 -- Number of random configs to test
    ...
    local tip=sim.getObjectHandle('RDS_01_tip')
    local base=sim.getObjectHandle('RDS_01')
    local robotCollection=sim.getCollectionHandle('RDS_01')
    local points={}
    local normals={}
    ...
    if extractConvexHull then
        ...
        sim.setObjectName(shape,'workspace')
    end
end
