...
-- 检测余下路径是否与目标是否有碰撞,返回碰撞构型的绝对和相对序号
--进行障碍物检查逻辑
...
if (sim_call_type==sim_childscriptcall_actuation) then   
    ...
    if res==1 then
        ...
        if minDist<THRESHOLD_DISTANCE then
            nindex,nconfigs=checkCollision() --通过jac_检测是否会碰撞上,返回碰撞的绝对构型步数和相对构型步数
            if nconfigs and nconfigs>0  then
                simAddStatusbarMessage(string.format("检测到规划路径上有干涉,绝对位置:%d 步,相对位置:%d 步",nindex,nconfigs))
                if nconfigs<=STOP_RATIO*CONFIG_NUM then--如果相对序号小于阈值
                    --command=simGetStringSignal("command")
                    if command=="start" then--当之前姿态为start时表示正常运行
                        lastTime=simGetSimulationTime()--获取停止命令时的时刻
                        simSetStringSignal("command","stop")--这段代码会导致函数executeMotion空转，即实现暂停动作
                    end
                    thisTime=simGetSimulationTime()
                    --如果暂停的时间大于REPLAN_INTERVAL，则重新规划路径
