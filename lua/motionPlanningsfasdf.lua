...
findSeveralCollisionFreeConfigs=function(matrix,trialCnt,maxConfigs)
    ...
    local cc=getConfig()
    local cs={}
    local l={}--初始化存储找到构型与当前构型的距离
    simAddStatusbarMessage("开始寻找可用的目标构型:")
    for i=1,trialCnt,1 do  --寻找目标构型循环次数为trialCnt
        --寻找最多maxConfigs个目标构型
        ...
        if c then
            ...
            for j=1,#l,1 do
                if math.abs(l[j]-dist)<0.001 then--先检查构型距离（cc-c）是否低于阈值
                    same=true
                    for k=1,#jh,1 do
                        if math.abs(cs[j][k]-c[k])>0.01 then--当距离低于阈值后，再检查各对应关节和是否低于阈值
                            same=false
                            break
                        end
                    end
...
executeMotion=function(path,lengths,maxVel,maxAccel,maxJerk,configNumer_1)
    ...
    while true do
        posVelAccel={0,0,0}--初始化需要生成的pos,vel,accel
        targetPosVel={lengths[#lengths],0}--定义目标末端pos,vel
        pos=0
        res=0
        ...
        while res==0 do
            ...
            if (res>=0) then
                l=posVelAccel[1]--提取当前位置pos
                for i=1,#lengths-1,1 do--与输入离散lengths对于前后点进行比较,计算比例
                    l1=lengths[i]
                    l2=lengths[i+1]
                    if (l>=l1)and(l<=l2) then
                        t=(l-l1)/(l2-l1)
                        for j=1,6,1 do
                            q=path[6*(i-1)+j]+_getJointPosDifference(path[6*(i-1)+j],path[6*i+j],jt[j]==sim_joint_revolute_subtype)*t--计算具体q值
                            dq=_getJointPosDifference(previousQ[j],q,jt[j]==sim_joint_revolute_subtype)--当前速度
                            previousQ[j]=q
                            r=math.abs(dq/dt)/jointsUpperVelocityLimits[j]--计算当前速度与限制速度比例
                            if (r>rMax) then
                                rMax=r--更新rMax
                            end
                        end
    ...
    while res==0 do
        ...
        if (res>=0) then
            ...
            for i=1,#lengths-1,1 do
                ---------------设置当前的构型的步数-----------
                --当command!="start"时执行轨迹动作，如replan或stop等命令
                repeat
                ...
                until(command=="start")
                simSetIntegerSignal(configNumer_1,i)--发布当前走到的构型
                -----------------------------------------
                l1=lengths[i]
                l2=lengths[i+1]
                if (l>=l1)and(l<=l2) then
